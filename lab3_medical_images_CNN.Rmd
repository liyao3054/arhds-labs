...
Diabetic retinopathy (DR) is a diabetes-related eye disease that damages the retina and can cause vision loss if left untreated. Automatically detecting referable DR from retinal photographs can improve screening efficiency, expand access to care, and enable earlier treatment. Screening tools often favor over-referral because missing a patient with sight-threatening disease is more harmful than unnecessarily referring someone, making high sensitivity a priority.
...
One missing piece of information is patient characteristics, such as age or sex. These factors may influence the appearance and prevalence of diabetic retinopathy, and without them it is difficult to assess whether the results are generalizable to different populations or whether the model performs equally well across subgroups. It also does not specify which eye (left or right) each image represents, or whether multiple images come from the same patient. This is important because treating images as independent observations when they come from the same individual can lead to data leakage and overly optimistic model performance estimates.
...
A major issue is that the model can become biased toward the majority class (healthy subjects). Because predicting “healthy” is often correct more frequently, a classifier may learn to favor that output and still achieve high overall accuracy while performing poorly at detecting disease. Also, the model may have difficulty learning patterns of the minority class due to limited examples, which can result in poor generalization for detecting patients with the disease. This is especially problematic in medical settings where the minority class (patients) is often the most clinically important.
...
1.  Pixel intensity distribution. In the same class, some images looks brighter than others, which suggests that those images have higher intensities in general.
2.  Color contrast. Some images look yellow. Some look red.

Grade 0 images look clearer whereas grade 4 images look cloudy showing more obvious abnormalities.
...
16 is batch size. 3 means 3 channels for each image. 244 and 244 are the size of each image. The photographs are colored image and the three channels are RGB values for images. It would be 16, 1, 1000, 1000, 1000.
...
If we use statistics from the full dataset, information from the test and validation sets is indirectly used during training. This leaks knowledge about the distribution of unseen data into the preprocessing step, meaning the model is no longer being evaluated on truly “unseen” data. As a result, performance estimates become overly optimistic because the model benefits from prior exposure to test-set characteristics.
...
No, although the training accuracy increases slightly during the first epoch and both training and validation losses decrease, the training and validation accuracies remain essentially unchanged. This suggests that the model may be becoming more confident in its predictions without actually improving its ability to correctly distinguish between the two classes. The lack of improvement in validation accuracy indicates that the network is not learning useful features that generalize to unseen data.
...
Because the left and right eyes are mirror images of each other. Flipping an image horizontally changes the relative positions of anatomical structures such as the optic disc and macula, creating images that may not occur naturally and potentially confusing the model.
...
# Uncomment, complete the rotation, and rebuild the augmented training dataloader.
 train_transform <- function(path) {
   base_loader(path) |>
     transform_to_tensor() |>
     transform_random_resized_crop(size = c(224, 224), scale = c(0.8, 1.0)) |>
     transform_random_rotation(degrees = 10) |>
     transform_color_jitter(brightness = 0.2, contrast = 0.2) |>
     transform_normalize(mean = imagenet_mean, std = imagenet_std)
 }
```
...
Data augmentation did not make a difference in this case. The validation accuracy curves for the augmented and un-augmented models remained unchanged, and the validation losses decreased by similar amounts. This suggests that augmentation did not noticeably improve the model's ability to distinguish between two classes. One possible reason is that the dataset and network are both relatively small. The network may lack the capacity to learn complex features that would benefit from augmentation, and the limited amount of data may mean that the model is underfitting rather than overfitting.
...
ImageNet is a large image dataset containing over one million images across 1000 object categories. Networks trained on ImageNet learn general visual features such as edges, textures, shapes, and patterns. Although retinal photographs are very different from images of cats, cars, or mushrooms, the early convolution and pooling layers capture these low-level features, which are useful for many image recognition tasks. By reusing these learned features, transfer learning allows the model to start from a strong foundation rather than learning everything from scratch, improving performance and reducing the amount of retinal image data required to train an accurate diabetic retinopathy classifier.
...
Sensitivity is the proportion of people with the disease who are correctly identified by the test (true positive rate), while specificity is the proportion of people without the disease who are correctly identified as disease-free (true negative rate). Increasing sensitivity reduces the number of missed cases but often leads to more false positives and lower specificity. Conversely, increasing specificity reduces false positives but may increase the number of missed cases. In a clinical screening program, sensitivity is generally more important because missing a patient with disease can delay treatment and lead to serious consequences.

The perfect sensitivity would lose specificity of 0.53. The perfect specificity would only have 0.35 sensitivity.
...
Attribution or saliency maps provide evidence that the model is making decisions based on clinically relevant features rather than simply achieving good performance metrics. While accuracy, sensitivity, and specificity indicate how well a model performs, they do not reveal why the model makes its predictions. Saliency maps can show whether the model is focusing on lesions such as microaneurysms, haemorrhages, and exudates, which are the same features used by clinicians. A model could achieve high performance for the wrong reason if it learns to rely on spurious cues in the images, such as differences in image quality, camera artifacts, or labels that are correlated with disease status. An attribution map could expose this by showing that the model is focusing on these irrelevant regions rather than on retinal lesions, indicating that its predictions may not generalize reliably in clinical practice.
...
Because it tests whether a model generalises beyond the specific conditions it was trained and tuned on. A held-out split from the same dataset still shares the same population, imaging devices, acquisition protocols, and potential dataset-specific biases, so performance can be overly optimistic. In contrast, validating on an external dataset introduces variation in patient population, cameras, and imaging protocols. This makes it much harder for the model to rely on dataset-specific shortcuts and provides a more realistic assessment of whether it will perform well in real-world clinical settings. As a result, strong performance on external data is more convincing evidence of true generalisability and clinical usefulness.
