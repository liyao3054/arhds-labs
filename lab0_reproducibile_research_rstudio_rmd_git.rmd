...
1846 rows and 3 columns. They are dataset, x and y.
...
Despite the differences of the shapes, the groups of data could have the same statistical summaries as they only capture parts of the features about data. This underscores a critical lesson: statistical summaries alone are insufficient; visualization techniques like histograms, boxplots, or density curves are essential.
...
aq |> filter(!is.na(Ozone)) |> 
  mutate(month_name = month.abb[Month]) -> aq_processed
...
```{r}
aq_processed |> group_by(Month)  |> summarise(mean(Ozone), mean(Temp), n())
```

```{r}
ggplot(aq_processed, aes(x=Ozone, y=Temp, color=Month)) + geom_point() + geom_smooth(method = "lm", data = aq_processed, mapping = aes(x=Ozone, y=Temp))
```
...
github link: <https://github.com/liyao3054/arhds-labs>
